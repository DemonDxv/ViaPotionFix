...
After digging deep into the packet protocol, and deleting multiple plugins, I came to the realization it was **ViaVersion** all along.
...
## The Solution
[**ViaPotionFix**](https://github.com/DemonDxv/ViaPotionFix/releases/tag/Release) corrects this translation issue at the packet level using PacketEvents.
With PacketEvents, we intercept ``CREATIVE_INVENTORY_ACTION`` and ``PLAYER_BLOCK_PLACEMENT`` packets, and if a potion contains the corrupted **ViaVersion** NBT tag, it is completely removed out before the server processes the packet.
